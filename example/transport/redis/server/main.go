...
func main() {
	...
	server.SetTransportRequestHandler(func(clientID string, request ocpp.Request, requestId string, action string) {
		...
		case *core.BootNotificationRequest:
			...
			log.Printf("Sending BootNotificationConfirmation response [%s] to client %s", requestId, clientID)
			if err := server.SendResponse(clientID, requestId, response); err != nil {
				log.Printf("Error sending response: %v", err)
			} else {
				log.Printf("Successfully sent BootNotificationConfirmation [%s]", requestId)
			}
		case *core.HeartbeatRequest:
			// Respond to heartbeat
			currentTime := types.NewDateTime(time.Now())
			response := core.NewHeartbeatConfirmation(currentTime)
			log.Printf("Sending HeartbeatConfirmation response [%s] to client %s", requestId, clientID)
			if err := server.SendResponse(clientID, requestId, response); err != nil {
				log.Printf("Error sending response: %v", err)
			} else {
				log.Printf("Successfully sent HeartbeatConfirmation [%s]", requestId)
			}
		case *core.StatusNotificationRequest:
			// Respond to status notification
			response := core.NewStatusNotificationConfirmation()
			log.Printf("Sending StatusNotificationConfirmation response [%s] to client %s", requestId, clientID)
			if err := server.SendResponse(clientID, requestId, response); err != nil {
				log.Printf("Error sending response: %v", err)
			} else {
				log.Printf("Successfully sent StatusNotificationConfirmation [%s]", requestId)
			}
		default:
	...
	log.Printf("Starting OCPP server with Redis transport at %s...", redisAddr)
	go func() {
	...
	log.Println("OCPP server started and listening for messages via Redis transport")

	// Send periodic remote start transactions to demonstrate server->client communication via Redis
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				// Get list of connected clients via Redis transport
				clients := redisTransport.GetConnectedClients()
				if len(clients) > 0 {
					clientID := clients[0] // Use first client
					log.Printf("Sending remote start transaction to client %s via Redis", clientID)

					// This would require implementing the remote start request
					// For now, just log that we would send it via Redis transport
					log.Printf("Would send RemoteStartTransaction to client %s via Redis transport", clientID)
				}
			}
		}
	}()
	...
	log.Println("Shutting down OCPP server...")

	// Stop Redis transport server gracefully
	shutdownCtx, shutdownCancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer shutdownCancel()

	if err := server.StopWithTransport(shutdownCtx); err != nil {
		log.Printf("Error stopping Redis transport server: %v", err)
	}

	log.Println("OCPP server stopped")
}